...
concat(CONVERT(VARCHAR(15), CAST(REPLACE(w.max_end_date, 12, 12) AS DATETIME2), 104), '') as end_date,
  	col.position_name as posinition_name
from 
	active_learnings w
...
left join collaborators col on w.person_id = col.id
where gcs.group_id = " + {PARAM2} 
+ ({PARAM3}? "":"and course_id = " + {PARAM1} + " ") + 
"group by person_fullname, ass.name, w.start_learning_date, w.max_end_date, col.position_name
union
select 
...
left join collaborators col on q.person_id = col.id
where gcs.group_id = " + {PARAM2} 
+ ({PARAM3}? "":"and course_id = " + {PARAM1} + " ") + 
"group by person_fullname, ass.name, q.start_learning_date, q.max_end_date, col.position_name
";
